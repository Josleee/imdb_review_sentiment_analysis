...
class SyntaxParser:
    def __init__(self, path=paths.STANFORD_PATH, verbose=False, dependencies=False):
        ...
        if not self.dependencies:
            cmd = "java -mx1500m -cp \"%s/stanford-parser.jar\" edu.stanford.nlp.parser.lexparser.LexicalizedParser -retainTMPSubcategories -outputFormat \"penn\" \"%s/englishPCFG.ser.gz\" -" % (
            self.path, self.path)
        else:
            ...
            cmd = "java -mx1500m -cp \"%s/stanford-parser.jar\" edu.stanford.nlp.parser.lexparser.LexicalizedParser -retainTMPSubcategories -outputFormat \"penn, typedDependencies\" \"%s/englishPCFG.ser.gz\" -" % (
            self.path, self.path)
