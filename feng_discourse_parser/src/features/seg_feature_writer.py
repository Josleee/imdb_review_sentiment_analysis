...
class SegFeatureWriter:
    ...
    def get_word_context(self, leaf_index, t):
        ...
        # print Nw.parent()
        while Nw.parent() != None and Nw.parent().parent() != None and t.leaves()[Nw.parent().head] == t.leaves()[
            leaf_index] and Nw.parent().right_sibling() != None:
            Nw = Nw.parent()
        ...
        for t in trees:
            ...
            for w in t.leaves():
                ...
                if align_with_string:
                    ...
                    w_lookup_regex = w_pre_regex.replace("\\", u"\\\\").replace(".", u"\.").replace("^", u"\^").replace(
                    ...
                    if m:
                        ...
                        t.draw()
                        print "Oops, couldn't match the .edu file and PTB tree for %s, faulty word is %s (was searched with regex %s). Word trace is %s" % (
                        w, w_pre_regex, w_lookup_regex, word_trace)
                        raise Exception
                ...
                if not prev_w_path == None:
                    ...
                    prev_cur_anc_pos = utils.utils.simplified_tag(
                        reduce(lambda x, y: x[y], [t] + prev_cur_anc_path).node)

                    if prev_cur_anc_pos in self.pos_dict:
                        features["____POS_tag_ancestor_previous_current_word"][
                            self.pos_dict.index(prev_cur_anc_pos)] = 1
                ...
                if not next_w_path == None:
                    ...
                    cur_next_anc_pos = utils.utils.simplified_tag(
                        reduce(lambda x, y: x[y], [t] + cur_next_anc_path).node)

                    if cur_next_anc_pos in self.pos_dict:
                        features["____POS_tag_ancestor_current_next_word"][
                            self.pos_dict.index(utils.utils.simplified_tag(cur_next_anc_pos))] = 1
