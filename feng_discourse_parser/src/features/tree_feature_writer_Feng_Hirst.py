...
class TreeFeatureWriter:
    def __init__(self, verbose=False, use_word_pairs=True, use_word_dep_types=True, use_syntax_production_rules=True,
        ...
        if self.use_RST_production_rules:  # or self.use_contextual_features:
            ...
            if self.use_RST_production_rules:
                ...
                #                            RST_pr_id += 1
                self.RST_production_rules_L = utils.serialize.loadData(
                    os.path.join(subdir, 'RST_production_rules_selected_L'))
                ...
                self.features.add_group('RST_Production_Rule_L', len(self.RST_production_rules_L),
                                        self.RST_production_rules_legend_L)

                self.RST_production_rules_R = utils.serialize.loadData(
                    os.path.join(subdir, 'RST_production_rules_selected_R'))
                ...
                self.features.add_group('RST_Production_Rule_R', len(self.RST_production_rules_R),
                                        self.RST_production_rules_legend_R)

                self.RST_production_rules_both = utils.serialize.loadData(
                    os.path.join(subdir, 'RST_production_rules_selected_both'))
                ...
                self.features.add_group('RST_Production_Rule_both', len(self.RST_production_rules_both),
                                        self.RST_production_rules_legend_both)

                if self.use_pair_features:
                    self.RST_production_rules_L_R = utils.serialize.loadData(
                        os.path.join(subdir, 'RST_production_rule_pairs_selected'))
                    ...
                    self.features.add_group('RST_Production_Rule_L_R', len(self.RST_production_rules_L_R),
                                            self.RST_production_rules_legend_L_R)
                ...
                if verbose:
                    ...
                    if self.use_pair_features:
                        print 'Loaded RST production rules list for left-right span: ' + str(
                            len(self.RST_production_rules_L_R))
        ...
        if self.use_syntax_production_rules:
            ...
            self.syntax_production_rules_L = utils.serialize.loadData(
                os.path.join(subdir, "syntax_production_rules_selected_L"))
            ...
            self.features.add_group('Syntax_Production_Rule_L', len(self.syntax_production_rules_L),
                                    self.syntax_production_rules_legend_L)

            self.syntax_production_rules_R = utils.serialize.loadData(
                os.path.join(subdir, "syntax_production_rules_selected_R"))
            ...
            self.features.add_group('Syntax_Production_Rule_R', len(self.syntax_production_rules_R),
                                    self.syntax_production_rules_legend_R)

            self.syntax_production_rules_both = utils.serialize.loadData(
                os.path.join(subdir, "syntax_production_rules_selected_both"))
            ...
            self.features.add_group('Syntax_Production_Rule_both', len(self.syntax_production_rules_both),
                                    self.syntax_production_rules_legend_both)

            if self.use_pair_features:
                self.syntax_production_rule_pairs = utils.serialize.loadData(
                    os.path.join(subdir, "syntax_production_rule_pairs_selected"))
                self.syntax_production_rule_pairs_legend = {}
                for production_pair in self.syntax_production_rule_pairs:
                    # print production_pair
                    self.syntax_production_rule_pairs_legend[
                        self.syntax_production_rule_pairs[production_pair]] = production_pair
                #            index = 0
                ...
                self.features.add_group('Syntax_Production_Rule_L_R', len(self.syntax_production_rule_pairs),
                                        self.syntax_production_rule_pairs_legend)
                if verbose:
        ...
        if self.use_ngram_features:
            ...
            self.features.add_group('Main_Nucleus_NGrams_pref_both', len(self.ngram_both_pref),
                                    self.ngram_both_pref_legend)
            if verbose:
            ...
            self.features.add_group('Main_Nucleus_NGrams_suf_both', len(self.ngram_both_suf),
                                    self.ngram_both_suf_legend)
            if verbose:
                print "Loaded ngram dict_suf_both: " + str(len(self.ngram_both_suf))

            if self.use_pair_features:
                self.ngram_l_pref_r_pref = utils.serialize.loadData(
                    os.path.join(subdir, 'ngram_pairs_selected_pref_L_pref_R'))
                ...
                self.features.add_group('NGrams_pref_L_pref_R', len(self.ngram_l_pref_r_pref),
                                        self.ngram_l_pref_r_pref_legend)
                self.features.add_group('Main_Nucleus_NGrams_pref_L_pref_R', len(self.ngram_l_pref_r_pref),
                                        self.ngram_l_pref_r_pref_legend)
                if verbose:
                    print 'Loaded ngram dict pref_L_pref_R:', len(self.ngram_l_pref_r_pref)

                self.ngram_l_pref_r_suf = utils.serialize.loadData(
                    os.path.join(subdir, 'ngram_pairs_selected_pref_L_suf_R'))
                ...
                self.features.add_group('NGrams_pref_L_suf_R', len(self.ngram_l_pref_r_suf),
                                        self.ngram_l_pref_r_suf_legend)
                self.features.add_group('Main_Nucleus_NGrams_pref_L_suf_R', len(self.ngram_l_pref_r_suf),
                                        self.ngram_l_pref_r_suf_legend)
                if verbose:
                    print 'Loaded ngram dict pref_L_suf_R:', len(self.ngram_l_pref_r_suf)

                self.ngram_l_suf_r_pref = utils.serialize.loadData(
                    os.path.join(subdir, 'ngram_pairs_selected_suf_L_pref_R'))
                ...
                self.features.add_group('NGrams_suf_L_pref_R', len(self.ngram_l_suf_r_pref),
                                        self.ngram_l_suf_r_pref_legend)
                self.features.add_group('Main_Nucleus_NGrams_suf_L_pref_R', len(self.ngram_l_suf_r_pref),
                                        self.ngram_l_suf_r_pref_legend)
                if verbose:
                    print 'Loaded ngram dict suf_L_pref_R:', len(self.ngram_l_suf_r_pref)

                self.ngram_l_suf_r_suf = utils.serialize.loadData(
                    os.path.join(subdir, 'ngram_pairs_selected_suf_L_suf_R'))
                ...
                self.features.add_group('NGrams_suf_L_suf_R', len(self.ngram_l_suf_r_suf),
                                        self.ngram_l_suf_r_suf_legend)
                self.features.add_group('Main_Nucleus_NGrams_suf_L_suf_R', len(self.ngram_l_suf_r_suf),
                                        self.ngram_l_suf_r_suf_legend)
                if verbose:
        ...
        if self.use_lexical_heads:
            ...
            self.features.add_group('Top_lexical_head_both', len(self.lexical_heads_both),
                                    self.lexical_heads_legend_both)
            if verbose:
                print 'Loaded lexical head list both:', len(self.lexical_heads_both)

            self.lexical_heads_dominated = utils.serialize.loadData(
                os.path.join(subdir, 'lexical_heads_selected_dominated'))
            ...
            self.features.add_group('Dominated_lexical_head', len(self.lexical_heads_dominated),
                                    self.lexical_heads_legend_dominated)
            if verbose:
        ...
        if self.use_syntactic_tags or self.use_top_syntactic_tags:
            if self.use_syntactic_tags:
                self.syntactic_tags_pref_L = utils.serialize.loadData(
                    os.path.join(subdir, 'syntactic_tags_selected_pref_L'))
                ...
                self.features.add_group('Syntactic_tags_pref_L', len(self.syntactic_tags_pref_L),
                                        self.syntactic_tags_pref_L_legend)
                if verbose:
                    print 'Loaded syntactic tag pref L:', len(self.syntactic_tags_pref_L)

                self.syntactic_tags_pref_R = utils.serialize.loadData(
                    os.path.join(subdir, 'syntactic_tags_selected_pref_R'))
                ...
                self.features.add_group('Syntactic_tags_pref_R', len(self.syntactic_tags_pref_R),
                                        self.syntactic_tags_pref_R_legend)
                if verbose:
                    print 'Loaded syntactic tag pref R:', len(self.syntactic_tags_pref_R)

                self.syntactic_tags_pref_both = utils.serialize.loadData(
                    os.path.join(subdir, 'syntactic_tags_selected_pref_both'))
                ...
                self.features.add_group('Syntactic_tags_pref_both', len(self.syntactic_tags_pref_both),
                                        self.syntactic_tags_pref_both_legend)
                if verbose:
                    print 'Loaded syntactic tag pref both:', len(self.syntactic_tags_pref_both)

                self.syntactic_tags_suf_L = utils.serialize.loadData(
                    os.path.join(subdir, 'syntactic_tags_selected_suf_L'))
                ...
                self.features.add_group('Syntactic_tags_suf_L', len(self.syntactic_tags_suf_L),
                                        self.syntactic_tags_suf_L_legend)
                if verbose:
                    print 'Loaded syntactic tag suf L:', len(self.syntactic_tags_suf_L)

                self.syntactic_tags_suf_R = utils.serialize.loadData(
                    os.path.join(subdir, 'syntactic_tags_selected_suf_R'))
                ...
                self.features.add_group('Syntactic_tags_suf_R', len(self.syntactic_tags_suf_R),
                                        self.syntactic_tags_suf_R_legend)
                if verbose:
                    print 'Loaded syntactic tag suf R:', len(self.syntactic_tags_suf_R)

                self.syntactic_tags_suf_both = utils.serialize.loadData(
                    os.path.join(subdir, 'syntactic_tags_selected_suf_both'))
                ...
                self.features.add_group('Syntactic_tags_suf_both', len(self.syntactic_tags_suf_both),
                                        self.syntactic_tags_suf_both_legend)
                if verbose:
                    print 'Loaded syntactic tag suf both:', len(self.syntactic_tags_suf_both)

                if self.use_pair_features:
                    self.syntactic_tag_pairs_pref_l_pref_r = utils.serialize.loadData(
                        os.path.join(subdir, 'syntactic_tag_pairs_selected_pref_L_pref_R'))
                    ...
                    self.features.add_group('Syntactic_tags_pref_L_pref_R', len(self.syntactic_tag_pairs_pref_l_pref_r),
                                            self.syntactic_tag_pairs_pref_l_pref_r_legend)
                    if verbose:
                        print 'Loaded syntactic tag pairs pref L pref R:', len(self.syntactic_tag_pairs_pref_l_pref_r)

                    self.syntactic_tag_pairs_pref_l_suf_r = utils.serialize.loadData(
                        os.path.join(subdir, 'syntactic_tag_pairs_selected_pref_L_suf_R'))
                    ...
                    self.features.add_group('Syntactic_tags_pref_L_suf_R', len(self.syntactic_tag_pairs_pref_l_suf_r),
                                            self.syntactic_tag_pairs_pref_l_suf_r_legend)
                    if verbose:
                        print 'Loaded syntactic tag pairs pref L suf R:', len(self.syntactic_tag_pairs_pref_l_suf_r)

                    self.syntactic_tag_pairs_suf_l_pref_r = utils.serialize.loadData(
                        os.path.join(subdir, 'syntactic_tag_pairs_selected_suf_L_pref_R'))
                    ...
                    self.features.add_group('Syntactic_tags_suf_L_pref_R', len(self.syntactic_tag_pairs_suf_l_pref_r),
                                            self.syntactic_tag_pairs_suf_l_pref_r_legend)
                    if verbose:
                        print 'Loaded syntactic tag pairs suf L pref R:', len(self.syntactic_tag_pairs_suf_l_pref_r)

                    self.syntactic_tag_pairs_suf_l_suf_r = utils.serialize.loadData(
                        os.path.join(subdir, 'syntactic_tag_pairs_selected_suf_L_suf_R'))
                    ...
                    self.features.add_group('Syntactic_tags_suf_L_suf_R', len(self.syntactic_tag_pairs_suf_l_suf_r),
                                            self.syntactic_tag_pairs_suf_l_suf_r_legend)
                    if verbose:
            ...
            if self.use_top_syntactic_tags:
                ...
                self.top_syntactic_tags_L = utils.serialize.loadData(
                    os.path.join(subdir, 'top_syntactic_tags_selected_L'))
                ...
                self.features.add_group('Top_Syntactic_tag_L', len(self.top_syntactic_tags_L),
                                        self.top_syntactic_tags_L_legend)
                if verbose:
                    print 'Loaded top syntactic tag list L:', len(self.top_syntactic_tags_L)

                self.top_syntactic_tags_R = utils.serialize.loadData(
                    os.path.join(subdir, 'top_syntactic_tags_selected_R'))
                ...
                self.features.add_group('Top_Syntactic_tag_R', len(self.top_syntactic_tags_R),
                                        self.top_syntactic_tags_R_legend)
                if verbose:
                    print 'Loaded top syntactic tag list R:', len(self.top_syntactic_tags_R)

                self.top_syntactic_tags_both = utils.serialize.loadData(
                    os.path.join(subdir, 'top_syntactic_tags_selected_both'))
                ...
                if self.use_pair_features:
                    self.top_syntactic_tags_L_R = utils.serialize.loadData(
                        os.path.join(subdir, 'top_syntactic_tags_selected_L_R'))
                    ...
                    self.features.add_group('Top_Syntactic_tag_L_R', len(self.top_syntactic_tags_L_R),
                                            self.top_syntactic_tags_L_R_legend)
                    if verbose:
                        print 'Loaded top syntactic tag list L R:', len(self.top_syntactic_tags_L_R)

                self.top_syntactic_tags_dominated = utils.serialize.loadData(
                    os.path.join(subdir, 'top_syntactic_tags_selected_dominated'))
                ...
                self.features.add_group('Top_Syntactic_tag_dominated',
                                        len(self.top_syntactic_tags_dominated),
                                        self.top_syntactic_tags_dominated_legend)
                if verbose:
    ...
    def write_position_features(self, syntax_trees, breaks, num_edus, suffix, start_i, start_j):
        ...
        while start_word < len(syntax_trees[start_i].leaves()) and not syntax_trees[start_i].leaves()[start_word].strip(
                exclude_words):
            start_word += 1
        tot = 0
        i = start_i
        ...
        if end_i == start_i:
            ...
            self.features['Sentence_tokens_cover_' + suffix] = float(
                breaks[end_i][end_j][0] - breaks[end_i][end_j][1]) / len(syntax_trees[start_i].leaves())
    ...
    def write_baseline_features(self, L, full_tree, syntax_trees, breaks, offsetL, offsetR):
        ...
        (l_end_i, l_end_j, l_end_word, l_ancestor_pos) = self.write_position_features(syntax_trees, breaks,
                                                                                      left_num_edus,
        ...
        if offsetR == 0:
            if l_end_j + 1 >= len(breaks[l_end_i]):
                if l_end_i + 1 >= len(syntax_trees):  ### DEBUG
                    ...
                    print syntax_trees[l_start_i].leaves()[
                          breaks[l_start_i][l_start_j][0]:breaks[l_start_i][l_start_j][1]]
        ...
        (r_end_i, r_end_j, r_end_word, r_ancestor_pos) = self.write_position_features(syntax_trees, breaks,
                                                                                      right_num_edus,
        ...
        if l_start_i == r_end_i:
            ...
            self.features['Sentence_tokens_cover_RL'] = float(len(left) + len(right)) / len(
                syntax_trees[l_start_i].leaves())
    ...
    def write_instance(self, mc_output, bin_output, L, prev_stump, next_stump, full_tree, syntax_trees, deps, breaks,
                       offsetL, offsetR=0,
        ...
        if self.use_lexical_heads or self.use_syntactic_tags or self.use_top_syntactic_tags:
            ...
            l_num_edus = 1 if not isinstance(L[0], Tree) else len(L[0].leaves())
            (l_ancestor_pos, l_pref_tag, l_suf_tag, l_top_tag) = self.get_conservative_top_ancestors(syntax_trees,
                                                                                                     breaks,
                                                                                                     l_start_i, l_end_i,
                                                                                                     l_start_j, l_end_j,
                                                                                                     l_num_edus, 'L')
            ...
            (r_ancestor_pos, r_pref_tag, r_suf_tag, r_top_tag) = self.get_conservative_top_ancestors(syntax_trees,
                                                                                                     breaks,
                                                                                                     r_start_i, r_end_i,
                                                                                                     r_start_j, r_end_j,
                                                                                                     r_num_edus, 'R')
    ...
    def write_word_dep_types_features(self, deps, syntax_trees, l_span_pos_list, r_span_pos_list):
        ...
        for l_span_pos in l_span_pos_list:
            left_word_dep_types.extend(
                self.write_word_dep_types_features_from_span(deps, syntax_trees, l_span_pos, 'L'))

        right_word_dep_types = []
        for r_span_pos in r_span_pos_list:
            right_word_dep_types.extend(
                self.write_word_dep_types_features_from_span(deps, syntax_trees, r_span_pos, 'R'))
    ...
    def write_word_dep_types_features_from_span(self, deps, syntax_trees, span_pos, suffix):
        ...
        if start_sent_id == end_sent_id:
            sent_deps = []
            for (dep_type, governor_word, governor_word_number, dependent_word, dependent_word_number) in deps[
                start_sent_id]:
                if governor_word_number >= start_word_offset and governor_word_number <= end_word_offset \
                    ...
                    sent_deps.append(
                        (dep_type, governor_word, governor_word_number, dependent_word, dependent_word_number))
            ...
            if self.use_pair_features:
                for production_r in right_production_list:
                    ...
                    if pair_production in self.syntax_production_rule_pairs:
                        self.features['Syntax_Production_Rule_L_R'][
                            self.syntax_production_rule_pairs[pair_production]] = 1
    ...
    def write_syntax_production_rules_from_span(self, syntax_trees, span_pos, suffix):
        ...
        if start_sent_id == end_sent_id:
            ...
            production_list = self.get_syntax_production_rules_from_subtrees(syntax_tree, start_word_offset,
                                                                             end_word_offset)
        else:
            syntax_tree = syntax_trees[start_sent_id]
            production_list.extend(self.get_syntax_production_rules_from_subtrees(syntax_tree, start_word_offset,
                                                                                  len(syntax_tree.leaves()) - 1))
            for i in range(start_sent_id + 1, end_sent_id):
                syntax_tree = syntax_trees[i]
                production_list.extend(
                    self.get_syntax_production_rules_from_subtrees(syntax_tree, 0, len(syntax_tree.leaves()) - 1))
                    ...
                    if suffix == 'R':
                        for j in range(len(start_leaf_pos) - 1, 0, -1):
                            if syntax_trees[start_i][start_leaf_pos[: j]].leaves()[0] != start_word_str \
                                    or len(
                                        syntax_trees[start_i][start_leaf_pos[: j]].leaves()) + start_word >= end_word:
                                break
