...
def get_heads(mrg, outfname=None):
    ...
    outfname = os.path.join(paths.PENN2MALT_PATH,
                            datetime.now().strftime('%Y_%m_%d_%H_%M_%S')) if outfname is None else outfname
...
def get_constituents(tree, multiEDU=False, rel=False, nuclearity=False, offset=0):
    ...
    constituents.append(
        (tree.node[: -6] if rel else None, tree.node[-6:] if nuclearity else None, offset, offset + len(tree.leaves())))
...
def get_adjacent_stump(prev_tree, i, scope, pos='L'):
    ...
    if (not prev_scope and scope) or contains_stump(''.join(str(x) for x in treepos), stump_treepos):
        ...
        while len(treepos) > 0:
            ...
            if (not parent_prev_scope and scope) or contains_stump(''.join(str(x) for x in parent_treepos),
                                                                   stump_treepos):
                break
...
def get_context_stumps(prev_tree, stumps, pair, i):
    ...
    # print 'R:', pair[1]
    next_stump = get_adjacent_stump(prev_tree, tot + (1 if not isinstance(pair, ParseTree) else len(pair.leaves())) - 1,
                                    scope, 'R')
